  ...
  <v-container>
        ...
        <v-card class="rounded-xl mb-5">
          ...
          <v-card-text>
            <v-row>
              ...
              <v-col cols="12" md="6" lg="4" class="border-s">
                <div class="d-flex flex-column justify-center align-center w-100">
                  ...
                  <v-btn v-if="user.getAuth" color="red" variant="flat" :disabled="!state.selectReserve" size="x-large" class="w-100" max-width="316"
                    @click="state.orderModal = true">
                    {{ !state.selectReserve ? 'ابتدا زمان را انتخاب کنید' : 'رزرو نوبت مشاوره' }}
                  </v-btn>
                  <v-btn v-else @click="handleLogin" class="w-100" max-width="316">ورود</v-btn>
                </div>
              </v-col>
            </v-row>
      ...
      <v-form @submit.prevent="postOrder" ref="verifyForm">
        <div class="ma-2">
          ...
          <div class="d-flex mt-5">
            ...
            <span class="mr-5"> هزینه قابل پرداخت: </span>
            <div class="d-block">
    ...
    <mj-dialog v-model="state.paymentModal" title="رسید پیگیری پرداخت" :action-btn="true" action-btn-text="پرداخت"
      ...
      <v-row class="pa-10 text-center">
        ...
        <v-col cols="12">
          <p>هزینه قابل پرداخت: {{ state.paymentPayload.data.amount.toLocaleString() }} تومان</p>
        </v-col>
      </v-row>
    </mj-dialog>

    <mj-dialog v-model="state.scoreModal" title="به متخصص امتیاز دهید" :action-btn="true" action-btn-text="ثبت"
