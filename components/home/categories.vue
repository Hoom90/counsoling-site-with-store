      ...
      <v-card-text class="pt-1 pt-md-5 pb-9">
        <v-row class="justify-center">
          <v-col cols="6" sm="4" md="2" v-for="(item, index) in state.categories">
            <nuxt-link :to="`/category/${item.id}/${item.title.replaceAll(' ','-')}`">
              <v-card flat variant="text"  style="height: 130px;" class="pa-3 h-100">
                <BaseImage :src="item.imageId" class="rounded-xl imageComp" :alt="item.title"></BaseImage>
                <h4>{{ item.title }}</h4>
              </v-card>
            </nuxt-link>
          </v-col>
